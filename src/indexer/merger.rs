...
use crate::{DocId, SegmentComponent, InvertedIndexReader};
use std::cmp;
use std::collections::HashMap;
use std::sync::Arc;


// Advance to the first non-deleted document and return the number of documents
fn compute_doc_freq(original_docset: &SegmentPostings, delete_bitset_opt: Option<&DeleteBitSet>) -> u32 {
    if let Some(delete_bitset) = delete_bitset_opt {
        // We need to copy the docset in order as we will iterate through it.
        let mut docset = original_docset.clone();
        let mut doc_freq = 0;
        loop {
            let doc = docset.doc();
            if doc == TERMINATED {
    ...
    fn write_postings_for_field(
        ...
        while merged_terms.advance() {
            ...
            for heap_item in merged_terms.current_kvs() {
                ...
                let doc_freq = compute_doc_freq(&segment_postings, segment_reader.delete_bitset());
                if doc_freq > 0u32 {
            ...
            // postings serializer.
            for (segment_ord, mut segment_postings) in segment_postings_containing_the_term {
